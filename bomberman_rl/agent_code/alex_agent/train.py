from agent_code.alex_agent.callbacks import A_TO_NUM, Q_func
from collections import namedtuple, deque
import pickle
from typing import List
from agent_code.alex_agent.event_functions import walked_towards_closest_coin
import events as e
from agent_code.alex_agent.callbacks import state_to_features
import numpy as np
...
def forest_update(self):
    ...
    for idx in np.arange(A_NUM):
        ...
        if len(self.feat_history[idx])>0:
            selection_mask = np.random.choice(np.arange(len(self.feat_history[idx])), size = BATCH_SIZE)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    # Store the model
    with open("alex-saved-model.pt", "wb") as file:
