...
from agent_code.my_agent.callbacks import A_TO_NUM, tabular_Q_func
...
def immediate_q_update(self, idx):
    ...
    next_q_value = tabular_Q_func(self, transition.next_state)
    ...
    self.betas[idx] = self.betas[idx] / np.linalg.norm(self.betas[idx])


def TD_response_func(self):
    transition = self.transitions[-1]
    next_q_value = tabular_Q_func(self, transition.next_state)
...
#         # print("targets: ", self.target_history.get_storage())
#         # print("feats: ", self.feat_history.get_storage())
...
#         self.betas[i] += self.betas[i] / np.linalg.norm(self.betas[i])
...
def game_events_occurred(
    ...
    if old_game_state is not None:
        ...
        # self.logger.info(f"self.history_history: {self.feat_history.get_storage()}")
        # self.logger.info(
