...
def gradient_update(self, idx):
    ...
    self.betas = [
        np.array(beta) / np.linalg.norm(np.array(self.betas), ord=2)
...
def potential_reward(self, old_game_state, new_game_state):
    old_pot = coin_potential(self, old_game_state) + bomb_potential(
        self, old_game_state
    )
    new_pot = coin_potential(self, new_game_state) + bomb_potential(
        self, old_game_state
    )
...
def coin_potential(self, game_state):
    ...
    if len(coins) < COIN_K:
        ...
        coins_feat = np.concatenate(
            (coin_xy_rel[sorted_index], np.zeros((COIN_K - len(coins), 2)))
        )
    else:
